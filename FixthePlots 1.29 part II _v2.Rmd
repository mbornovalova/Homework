...
p <- tribble(
  ...
  ggplot(p, aes(x, y, label = rownames(p))) + 
  geom_line() +
  geom_text(size = 10) 
...
p$cyl<-  as.factor(p$cyl)
  ...
  ggplot(p, aes(x = cyl, y = ..prop.., fill = transmission), stat = "prop") +
  geom_bar(colour="black") +
  scale_fill_discrete(name = "Transmission", labels = c("Automatic", "Manual")) +
    theme_minimal() 
  
ggplot(data=p, aes(cyl))+
  geom_bar(aes(fill=as.factor(transmission), y = (..count..)/sum(..count..)), position="dodge") + 
...
```{r}
hair$Hair<-  as.factor(hair$Hair)
ggplot(hair)+
  geom_bar(mapping = aes(x = Hair, y = n), stat = "identity")  
...
ggplot(hair_eye, aes(Hair, Eye, fill= n)) +
  geom_tile() 

ggplot(hair_eye, aes(Hair, Eye, label = n, fill= n)) +
  geom_tile(aes(colour = n)) +
   geom_text(size = 5) 
```
